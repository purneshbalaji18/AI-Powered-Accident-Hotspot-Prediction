...
def prepare_dashboard_data():
    """
    Prepare dashboard data from model outputs.
    ...
    """
    try:
        logger.info("Starting dashboard data preparation...")
        
        # Validate required files exist
        required_files = [
            f"{MODELS_DIR}/accidents_enriched.csv",
            f"{MODELS_DIR}/hotspot_clusters.csv",
            f"{MODELS_DIR}/city_stats.csv",
            f"{MODELS_DIR}/feature_importance.csv",
            f"{MODELS_DIR}/recommendations.csv",
            f"{MODELS_DIR}/model_metrics.json"
        ]
        ...
        logger.info("Loading model outputs...")
        ...
        # 2. Map data - accidents sample for performance
        ...
        logger.info(f"[OK] Dashboard data prepared: {len(hotspots)} hotspots, {len(map_accidents)} accident points")
        logger.info(f"[OK] Output saved to: {STATIC_DIR}/dashboard_data.json")
        return dashboard_data
    
    except FileNotFoundError as e:
        logger.error(f"File not found during dashboard preparation: {e}")
        raise
    except KeyError as e:
        logger.error(f"Missing expected data column: {e}")
        raise
    except Exception as e:
        logger.error(f"Unexpected error during dashboard preparation: {e}", exc_info=True)
        raise


if __name__ == "__main__":
    try:
        logger.info("Starting dashboard data preparation process...")
        prepare_dashboard_data()
        logger.info("[SUCCESS] Dashboard preparation completed successfully")
    except FileNotFoundError as e:
        logger.error(f"Required output files not found. Ensure ML pipeline ran first: {e}")
        sys.exit(1)
    except Exception as e:
        logger.error(f"Failed to prepare dashboard data: {e}", exc_info=True)
        sys.exit(1)
